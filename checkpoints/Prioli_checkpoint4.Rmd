...
# Want to make a legend for the leaf colors; need to get this working

# ag_leg <- c("Underweight" = "#FFFF00",
...
# ag_legend <- tribble(
#   ~BMI_cat,          ~x,  ~y, ~lab_col,
#   "Underweight",      0,   0,  "#FFFF00",
#   "Healthy weight",   0.5, 0,  "#008318",
#   "Overweight",       1,   0,  "#FF8D03",
#   "Obese",            1.5, 0,  "#FF0000"
# ) %>% 
#   mutate(BMI_cat = factor(BMI_cat, levels = c("Underweight", "Healthy weight", "Overweight", "Obese")))
# 
# ag_legend_gg <- ggplot(data = ag_legend, aes(x = x, y = y)) +
#   geom_point(aes(color = "Underweight")) +
#   geom_point(aes(color = "Healthy weight")) +
#   geom_point(aes(color = "Overweight")) +
#   geom_point(aes(color = "Obese")) +
#   scale_color_manual(name = "BMI Category", values = ag_leg) +
#   theme(text = element_text(face = "bold"),
#         axis.line = element_blank(),
#         axis.text.x = element_blank(),
#         axis.text.y = element_blank(),
...
ag_labcolors <- unsup_labs_reord_di
...
```{r agnes_dend_clust, echo = FALSE, warning = FALSE, paged.print = FALSE, fig.width = 11, fig.height = 11}
# 6 clusters for AGNES
ggthemr("flat", spacing = 0.1)

# Moved above to first dendrogram; can delete once I ensure all is working properly

# ag_labcolors <- unsup_labs_reord_ag %>% 
#   as_tibble() %>% 
#   mutate(x = row_number(),
#          labcolor = case_when(   # Have decided not to use text labels but keeping this here in case I change my mind
#            BMI_cat == "UW" ~ "#B0B000", # Old value dark purple: "#3F345E"
#            BMI_cat == "HW" ~ "#008318", # Old value slate bue: "#34495E"
#            BMI_cat == "OW" ~ "#FF8D03", # Old value dark green: "#355E44"
#            BMI_cat == "OB" ~ "#FF0000", # Old value dark rose "#5E343F"
#            ),
#          leafcolor = case_when(
#            BMI_cat == "UW" ~ "#FFFF00",
#            BMI_cat == "HW" ~ "#008318",
...
agnes_dend_clust <- nhanes_agnes %>% 
  as.dendrogram() %>% 
  set("leaves_pch", 19, col = ag_labcolors$leafcolor) %>% 
  set("leaves_col", ag_labcolors$leafcolor) %>% 
  set("branches_k_col", k = 6, c("#3498DB",
                                 "#2ECC71",
                                 "#F39C12",
                                 "#E74C3C",
                                 #"#E73CCA",
...
agnes_dend_clust_ggd <- agnes_dend_clust %>% 
  as.ggdend()

agnes_dend_clust_gg <- ggplot(agnes_dend_clust_ggd, labels = FALSE) +
  scale_y_reverse() +
  coord_polar(theta = "x") +
...
```{r diana_dend_clust, echo = FALSE, warning = FALSE, paged.print = FALSE, fig.width = 11, fig.height = 11}
# 5 clusters for DIANA
ggthemr("flat", spacing = 0.1)
...
diana_dend_clust_ggd <- diana_dend_clust %>% 
  as.ggdend()

diana_dend_clust_gg <- ggplot(diana_dend_clust_ggd, labels = FALSE) +
  scale_y_reverse() +
  coord_polar(theta = "x") +
  theme(axis.line = element_blank(),
        axis.text.x = element_blank(),
  ...
  xlab("") +
  ylab("") +
  ggtitle("DIANA Dendrogram, k = 5 Clusters")
diana_dend_clust_gg
