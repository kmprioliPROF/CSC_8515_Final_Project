...
AGNES relied on complete (i.e., maximum distance) linkage between clusters and yielded the following dendrogram:

```{r agnes_dend, echo = FALSE, paged.print = FALSE, fig.width = 12, fig.height = 4}
agnes_dend <- agnes(x = (dissim_mat %>% as.matrix()), diss = TRUE, keep.diss = TRUE, method = "complete") %>%
  as.dendrogram() %>%
  ggdendrogram()
agnes_dend
```
