...
unsup_labs_reord <- unsup_labs[nhanes_agnes$order] %>%    # Reordering leaf labels
...
agnes_dend <- nhanes_agnes %>% 
  as.dendrogram() %>% 
  set("labels", unsup_labs_reord$BMI_cat)
...
The AGNES scree plot shows a relatively smooth curve without strong elbows; possible elbows occur at 4 and 7 clusters.

```{r silhou, echo = FALSE, paged.print = FALSE, fig.width = 10, fig.height = 6}
agnes_sil <- ggplot(data = agnes_screedata, aes(x = n_clusters, y = asw)) +
  geom_line() +
  geom_point() +
  xlab("Number of Clusters") +
