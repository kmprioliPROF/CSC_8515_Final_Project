...
```{python nxk_rkf}
rfc = ensemble.RandomForestClassifier(n_estimators = 200, min_samples_leaf = 25, bootstrap = False) #max_depth = 7, 
rkf = RepeatedKFold(n_splits = 20, n_repeats = 15)   # Reminder:  n = # splits = # folds; k = # repeats
...
```{r}
nhanes_BMIcat <- nhanes %>% 
  mutate(BMI_cat = factor(BMI_cat, levels = c(1, 2, 3, 4), labels = c("Underweight", "Healthy weight", "Overweight", "Obese")))

nhanes_rf <- randomForest(BMI_cat ~ ., data = nhanes_BMIcat)

round(importance(nhanes_rf), digits = 2)
```
