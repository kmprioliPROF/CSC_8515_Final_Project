...
# See checkpoints/rfc_f.dot; can plot this at webgraphviz.com

from sklearn.tree import export_graphviz

model = ensemble.RandomForestClassifier(n_estimators = 200, min_samples_leaf = 25, bootstrap = False, max_depth = 7)
model.fit(nhanes_feat_f, nhanes_labs_f.values.ravel())

est = model.estimators_[10]

diag = export_graphviz(est, out_file = "rfc_f2.dot", rounded = False, proportion = False, label = "none", impurity = "False", precision = 2, filled = True, feature_names = nhanes_feat_f.columns, class_names = ["Under/Normal Wt", "Overwt/Obese"])
```


```{python}

cross_val_score(model, nhanes_feat_f, nhanes_labs_f.values.ravel(), cv = 10)
```
