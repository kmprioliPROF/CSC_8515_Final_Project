...
scores_final <- py$scores_f %>% 
  ...
  summarize(n_runs = n(),
            pct_acc = round(mean(score), digits = 3) * 100,
            sd_acc = (round(sd(score), digits = 3) * 100),
            min_acc = round(min(score), digits = 3) * 100,
            max_acc = round(max(score), digits = 3) * 100)

scores_final_kbl <- scores_final %>% 
  kable(format = "markdown")

# save(scores_final, scores_final_kbl, file = "data/staging/random_forest_final_scores.RData")
```