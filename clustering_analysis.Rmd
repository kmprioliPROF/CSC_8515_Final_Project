...
ggleg <- ggplot(data = legend_tbl, aes(x = x, y = y)) +
  geom_point(aes(x = x, y = y, fill = BMI_cat), color = "black", shape = "circle filled", size = 5) +
  ...
  clusts <- cluster_clusts(df) %>% 
    ...
    mutate(pct_cat_in_clust = round((n_cat_in_clust / Size) * 100, digits = 1),
           ...
           BMI_cat = factor(BMI_cat, levels = c("Underweight (n, %)",
                                                ...
                                                "Overweight (n, %)",
                                                "Obese (n, %)"))) %>% 
    select(cluster_num, Size, BMI_cat, n_pct) %>% 
...
```{r agnes_dend_clust_ggd_final, echo = FALSE, warning = FALSE, paged.print = FALSE, fig.width = 6.5, fig.height = 6.5}
# 10 clusters for AGNES
agnes_dend_clust_final <- nhanes_agnes %>% 
  as.dendrogram() %>% 
  set("leaves_pch", 20, col = ag_labcolors$leafcolor) %>% 
  set("leaves_col", ag_labcolors$leafcolor) %>% 
  set("branches_lwd", 0.5) %>% 
...
agnes_dend_clust_final_ggd <- agnes_dend_clust_final %>% 
  as.ggdend()
```

```{r agnes_final_cluster_characteristics, echo = FALSE, warning = FALSE, paged.print = FALSE, fig.width = 10, fig.height = 4}
ag_final_clusts_df <- cluster_pct(agnes_dend_clust_final_ggd)

ag_final_clusts_kbl <- ag_final_clusts_df %>%
  rename(Cluster = cluster_num) %>%
  kable(format = "markdown")

ag_final_clust_labs <- cluster_labs(agnes_dend_clust_final_ggd)
```

```{r agnes_dend_clust_final, echo = FALSE, warning = FALSE, paged.print = FALSE, fig.width = 6.75, fig.height = 6.75}
ggthemr("flat", spacing = 0.1)

agnes_dend_clust_final_gg <- ggplot(agnes_dend_clust_final_ggd, labels = FALSE) +
  geom_text(data = ag_final_clust_labs, aes(x = x_lab, y = -0.5, label = cluster_num, color = cluster_col)) +
  scale_y_reverse() +
  coord_polar(theta = "x") +
  theme(axis.line = element_blank(),
        axis.text.x = element_blank(),
