...
* Began drafting presentation slides
...
```{python import_subset, eval = FALSE, include = FALSE, echo = FALSE, paged.print = FALSE, fig.width = 10, fig.height = 6}
nhanes_f = r.nhanes_sup_trim_dichot
nhanes_feat_f = nhanes_f.drop(["BMI_cat"], axis = 1)
nhanes_labs_f = nhanes_f.filter(["BMI_cat"])
```

```{python nxk_rkf, eval = FALSE, include = FALSE, echo = FALSE, paged.print = FALSE, fig.width = 10, fig.height = 6}
...
```{r score_rkf, eval = FALSE, include = FALSE, echo = FALSE, paged.print = FALSE, fig.width = 10, fig.height = 6}
...
agnes_dend_clust7 <- nhanes_agnes %>% 
  as.dendrogram() %>% 
  set("leaves_pch", 19, col = ag_labcolors$leafcolor) %>% 
  set("leaves_col", ag_labcolors$leafcolor) %>% 
...
agnes_dend_clust7_ggd <- agnes_dend_clust7 %>% 
  as.ggdend()

agnes_dend_clust7_gg <- ggplot(agnes_dend_clust7_ggd, labels = FALSE) +
  scale_y_reverse() +
  coord_polar(theta = "x") +
