...
cluster_clusts <- function(df, dichot = FALSE){   # dichot = 0 for 4-level BMI category, dichot = 1 for dichotomous; defaults to 4-level
  ...
  if(dichot == TRUE){
    leaves <- leaves %>% 
      mutate(
        BMI_cat = case_when(
          BMI_cat == "#0000FF" ~ "Not Overweight or Obese",
          BMI_cat == "#9C0022" ~ "Overweight or Obese"),
...
ag_dichot_clusts_df <- cluster_pct(agnes_dend_clust_dichot_ggd, dichot = TRUE)

ag_dichot_clusts_kbl <- ag_dichot_clusts_df %>%
  rename(Cluster = cluster_num) %>%
  kable(format = "markdown")
...
di_dichot_clusts_df <- cluster_pct(diana_dend_clust_dichot_ggd, dichot = TRUE)

di_dichot_clusts_kbl <- di_dichot_clusts_df %>% 
  rename(Cluster = cluster_num) %>% 
  kable(format = "markdown")
...
<!-- ANALYSIS WITH INCREASING k (OMITTING IN FAVOR OF DICHOTOMOUS ANALYSIS) -->
