...
```{r corrplots, echo = FALSE, paged.print = FALSE, fig.width = 9, fig.height = 6}
# Correlation plots for BMI_cat vs continuous and ordinal variables
...
unsup_labs_reord_ag <- unsup_labs[nhanes_agnes$order] %>%    # Reordering leaf labels
  ...
  mutate(x = row_number(),
         ...
         leafcolor_dichot = case_when(
           BMI_dichot == "UW/HW" ~ "#0000FF",         # Blue
           BMI_dichot == "OW/OB" ~ "#9C0022")) %>%    # Dark red
...
ggleg_dichot <- ggplot(data = legend_tbl, aes(x = x, y = y)) +
  geom_point(aes(x = x, y = y, fill = BMI_dichot), color = "#4A4A4A", shape = "circle filled", size = 3) +
...
cluster_clusts <- function(df, x = 0){   # x = 0 for 4-level BMI category, x = 1 for dichotomous; defaults to 4-level
  leaves <- df$nodes %>% 
    filter(leaf == TRUE) %>% 
    select(x, col) %>% 
    rename(BMI_cat = col) %>% 
    mutate(
      ...
      BMI_dichot = case_when(
        BMI_cat %in% c("Underweight", "Healthy Weight") ~ "Not Overweight \nor Obese",
        BMI_cat %in% c("Overweight", "Obese") ~ "Overweight \nor Obese")) %>% 
    mutate(BMI_cat = factor(BMI_cat, levels = c("Underweight", "Healthy Weight", "Overweight", "Obese")),
           BMI_dichot = factor(BMI_dichot, levels = c("Not Overweight \nor Obese", "Overweight \nor Obese")))
  
  if(x == 0){
    leaves <- leaves %>% select(x, BMI_cat)
  }
  
  if(x == 1){
    leaves <- leaves %>% select(x, BMI_dichot) %>% rename(BMI_cat = BMI_dichot)
  }
...
```{r cluster_num_labs}
cluster_labs <- function(df, x = 0){   # x = 0 for 4-level BMI category, x = 1 for dichotomous; defaults to 4-level
  clust_labs <- cluster_clusts(df, x) %>% 
...
cluster_pct <- function(df, x = 0){   # x = 0 for 4-level BMI category, x = 1 for dichotomous; defaults to 4-level
  clusts <- cluster_clusts(df, x) %>% 
  ...
  if(x == 1){
    clusts <- clusts %>% 
      mutate(BMI_cat = factor(BMI_cat, levels = c("Not Overweight \nor Obese (n, %)",
                                                  "Overweight \nor Obese (n, %)")))
  }
...
ag_init_clusts_df <- cluster_pct(agnes_dend_clust_init_ggd)

ag_init_clusts_kbl <- ag_init_clusts_df %>%
...
di_init_clusts_df <- cluster_pct(diana_dend_clust_init_ggd)

di_init_clusts_kbl <- di_init_clusts_df %>% 
  rename(Cluster = cluster_num) %>% 
  kable(format = "markdown")
...
```{r agnes_dend_clust_ggd_rev, echo = FALSE, warning = FALSE, paged.print = FALSE, fig.width = 6.5, fig.height = 6.5}
# 10 clusters for AGNES
agnes_dend_clust_rev <- nhanes_agnes %>% 
  as.dendrogram() %>% 
  set("leaves_pch", 20, col = ag_labcolors$leafcolor) %>% 
  set("leaves_col", ag_labcolors$leafcolor) %>% 
  set("branches_lwd", 0.5) %>% 
...
agnes_dend_clust_rev_ggd <- agnes_dend_clust_rev %>% 
  as.ggdend()
```

```{r agnes_rev_cluster_characteristics, echo = FALSE, warning = FALSE, paged.print = FALSE, fig.width = 10, fig.height = 4}
ag_rev_clusts_df <- cluster_pct(agnes_dend_clust_rev_ggd)

ag_rev_clusts_kbl <- ag_rev_clusts_df %>%
  rename(Cluster = cluster_num) %>%
  kable(format = "markdown")

ag_rev_clust_labs <- cluster_labs(agnes_dend_clust_rev_ggd)
```

```{r agnes_dend_clust_rev, echo = FALSE, warning = FALSE, paged.print = FALSE, fig.width = 6.75, fig.height = 6.75}
ggthemr("flat", spacing = 0.1)

agnes_dend_clust_rev_gg <- ggplot(agnes_dend_clust_rev_ggd, labels = FALSE) +
